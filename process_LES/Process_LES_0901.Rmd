---
title: "2022_0901_LES_data_analysis"
author: "Xin Tong"
date: "`r Sys.Date()`"
output:
  word_document:
...
filenames <- list.files(path = "F:/Harbour_les/les0901/", pattern = "")
...
for(i in 1:length(time)) {
  dat.nc <- nc_open(paste0("F:/Harbour_les/les0901/wrfout_d03_2022-09-06_", time[i]))
...
setwd("D:/1 PhD Studies/1 Data/LES/0901/wind")
...
pdf("D:/1 PhD Studies/1 Data/LES/0901/plots/mast.data.pdf")
...
dat.nc <- nc_open(paste0("F:/Harbour_les/les0901/wrfout_d03_2022-09-01_", time[1]))
...
for(i in seq.int(time)) {
  dat.nc <- nc_open(paste0("F:/Harbour_les/les0901/wrfout_d03_2022-09-01_", time[i]))
  ...
  df <- data.frame(
      datetime = as.POSIXct(paste0("2022-09-01 ", hr[i], ":", min[i], ":", sec[i]), tz = "UTC"),
...
write.csv(df, "D:/1 PhD Studies/1 Data/LES/0901/extract_les/wind.vertical.profile.csv")
...
df <- do.call(rbind, list)
write.csv(df, "D:/1 PhD Studies/1 Data/LES/0901/extract_les/meteo.vertical.profile.csv")
...
les <- read.csv("D:/1 PhD Studies/1 Data/LES/0901/wind/wind.vertical.profile.csv")
...
#pdf("D:/1 PhD Studies/1 Data/LES/0901/plots/mast&les.wind.pdf")
tiff(paste0("D:/1 PhD Studies/1 Data/LES/0901/plots/mast&les.wd.", lowest.level[1], ".tiff"), units="mm", width=300, height=200, res=300)
ggplot()+
...
les <- read.csv("D:/1 PhD Studies/1 Data/LES/0901/extract_les/meteo.vertical.profile.csv")
...
pdf("D:/1 PhD Studies/1 Data/LES/0901/plots/mast&les.meteo.pdf")
...
For the flights of 0901 and 0906, I selected several layers of les grid cells, which contain the min and max of altitude of airborne measurements. The analysis showed that there is not a significant difference between the estimated emissions using different layers of les grid cells. Hence, for the flight 0901, I only selected the layer of 25, which contains the altitude skyarrow flew the most.

```{r save the layers of 25 for wind as spatial vector file using terra package}
...
for(i in 50:73) {
  ...
  rm(alt_flat, U_flat, V_flat, alt_interpolated, U_interpolated, V_interpolated)
  
  # Define the layer name
  layer_name <- paste0("layer_", z)
  file_name <- paste0("D:/1 PhD Studies/1 Data/LES/0901/wind/wind_", time[i], ".gpkg")
...
Still use terra package! Use the function vect() to create a spatialvector object.

```{r save the layers of 25 for plume as spatial vector file using terra package}
...
for(i in 52:73) {
  ...
  # Define the layer name
  layer_name <- paste0("layer_", z)
  file_name <- paste0("D:/1 PhD Studies/1 Data/LES/0901/wind/plume_", time[i], ".gpkg")
...
```{r save the layers of 25 for pressure and temperature as spatial vector file using terra package}
...
for(i in 1:length(time)) {
  ...
  # Define the layer name
  layer_name <- paste0("layer_", z)
  file_name <- paste0("D:/1 PhD Studies/1 Data/LES/0901/wind/temp_", time[i], ".gpkg")
...
# pick up airborne measurements for harbour areas
data <- data[
  data$date>as.POSIXlt("2022-09-01 13:48:00", tz='UTC') & data$date<as.POSIXlt("2022-09-01 14:24:00", tz='UTC'),
  c("date", "lat", "lon", "alt", "N2O.ac.ppb", "CH4.ac.ppb")
  ]
names(data)[names(data)=="lat"] <- "lat.ac";
names(data)[names(data)=="lon"] <- "lon.ac";
...
les.date <- lubridate::ymd_hms(paste0("2022-09-01", hr, ":", min, ":", sec))
...
data.sel <- lapply(as.vector(les.date), function(daytime){
 ...
 # write.csv(data.sel, paste0("D:/1 PhD Studies/1 Data/LES/0901/2022_0901_aircore.sel.csv"), row.names = FALSE)
...
from <- as.POSIXct("2022-09-01 13:48:00", tz = "UTC");
to <- as.POSIXct("2022-09-01 14:24:00", tz = "UTC")
...
data.sel <- read.csv("D:/1 PhD Studies/1 Data/LES/0901/2022_0901_aircore.sel.csv")


results_list <- list()
for(i in seq.int(time)) {
    
  vec <- vect(paste0("D:/1 PhD Studies/1 Data/LES/0901/wind/wind_", time[i], ".gpkg"), layer = "layer_25")
  ...
  write.csv(df, paste0("D:/1 PhD Studies/1 Data/LES/0901/extract_les/wind.csv"), row.names = FALSE)


```

### **For each time stamp, extract the grid cells of LES matched with the flight trajectory for the one layer_25**
```{r }
...
# pick up airborne measurements for harbour areas
data <- data[data$time>(14*3600+5*60+16) & data$time<(14*3600+17*60), ]
...
# vec <- vect(paste0("D:/1 PhD Studies/1 Data/LES/0901/wind/plume_13_48_00.gpkg"), layer = "layer_25")
# crs(data.vec) <- crs(vec)
...
timestamps <- as.POSIXct(paste0("2022-09-01 ", hr, ":", min, ":", sec), tz = "UTC")

# pick up the data simulated by LES
results_list <- vector("list", length(time)) 
index <- 1
for(i in seq.int(time)){
    vec <- vect(paste0("D:/1 PhD Studies/1 Data/LES/0901/wind/plume_", time[i], ".gpkg"), layer = "layer_25")
    df <- as.data.frame(crop(vec, data.vec), geom = "XY")
    
    # remove unuseful objects from environments
    keep <- c( "i", "time", "timestamps", "data.vec", "results_list", "index", "df")
    rm(list = setdiff(ls(), keep))

    results_list[[index]] <- cbind(date = timestamps[i], layer = "layer_25", df)
    index <- index+1
...
write.csv(df.sel, "D:/1 PhD Studies/1 Data/LES/0901/extract_les/plume.layers.csv") # unit is ppb for both N2O and CH4


```

Whether aligning the crs of vec with the crs of data.vec, the trimmed area is not exactly the same to the smaller longitude of both of them. 
...
n2o <- read.csv("D:/1 PhD Studies/3.1 Results of projects/2022_Rotterdam_Campaign/Airborne_meas/0901/n2o.transect6.csv")  # flight 0901 does not show enhancements for ch4

# load LES data
les <- read.csv("D:/1 PhD Studies/1 Data/LES/0901/extract_les/plume.csv")
...
ws <- read.csv("D:/1 PhD Studies/1 Data/LES/0901/extract_les/wind.csv")
...
# convert the coordinates to distance along the trajectory
  lon.zero.6 <- 4.0199
  lat.zero.6 <- 51.9136
  coors <- data.frame(lon = lon.zero.6, lat = lat.zero.6)
...
distance <- ifelse(
  valid_coords,
  sqrt((lon.zero.6 - les$lon)^2 * per.lon(les$lat)^2 + (lat.zero.6 - les$lat)^2 * per.lat^2) *
    ifelse(les$lon > lon.zero.6, 1, -1),
  NA
)
...
#   n2o_valid$pre <- predict(fit, n2o_valid$distance)$y
# 
# 
  #   ggplot()+
  #   geom_point(n2o_valid, mapping = aes(x = distance, y = n2o))+
  #   geom_line(n2o_valid, mapping = aes(x = distance, y = pre), col = 'blue')+
  #   geom_smooth(n2o_valid, mapping = aes(x = distance, y = n2o), method = "loess", col = 'red', size = 0.1) +
  # 
  #   geom_point(les, mapping = aes(x = distance, y = N2O_flat))+
...
# load LES data
les <- read.csv("D:/1 PhD Studies/1 Data/LES/0901/extract_les/plume.layers.csv")
names(les)[names(les)=="x"] <- "lon"
names(les)[names(les)=="y"] <- "lat"
les <- les[les$lon>min(n2o$lon) & les$lon<max(n2o$lon) &les$lat>min(n2o$lat) & les$lat<max(n2o$lat),] # select the les grid cells corresponding to 6th transect (downwind transect)

ws <- read.csv("D:/1 PhD Studies/1 Data/LES/0901/extract_les/wind.layers.csv")
ws <- cbind(ws, ws = sqrt(ws$U_flat^2+ws$V_flat^2))
...
# check the fitted lines fro each time stamp
  # pdf("D:/1 PhD Studies/1 Data/LES/0901/plots/les.fit.pdf")
  ...
  # 
  # p.n2o <- ggplot()+
  #   geom_point(sel, mapping = aes(x = distance, y = N2O_flat))+
...
# create_plot <- function(df, y_var, y_pred, title_prefix) {
#   ggplot(df, aes(x = distance, colour = interval)) +
#     geom_point(aes_string(y = y_var), size = 0.1) +
#     geom_line(aes_string(y = y_pred, group = "interval")) +
#     geom_smooth(aes_string(y = y_var, group = "interval"), method = "loess", col = 'red', size = 0.1) +
#     #facet_grid(rows = vars(layer)) +
#     ggtitle(paste("red:LOESS & colourful:spline", title_prefix, sep = "_"))
# }
# 
...
#     pre.n2o = predict(smooth.spline(distance, n2o), distance)$y,
#     pre.ch4 = predict(smooth.spline(distance, ch4), distance)$y
#   ) %>%
#   ungroup() 
# 
#   # Generate the plots using the function and store them in the list
#   plot_list[[paste0("p.n2o_", i)]] <- create_plot(df.1, "n2o", "pre.n2o", i)
# }
# 
# # Arrange all plots into one figure and save it
# combined_plot <- marrangeGrob(plot_list, nrow = 2, ncol = 2)
# ggsave("D:/1 PhD Studies/1 Data/LES/0901/plots/les.fitting.plume.diff.interval.tiff", combined_plot, width = 14, height = 10)
# 
# while (!is.null(dev.list()))  dev.off()
####################################################################################
...
pdf("D:/1 PhD Studies/1 Data/LES/0901/plots/n2o.pdf")
  for(i in 1:5){
...
# pick up airborne measurements for harbour areas
data <- data[
  data$date>as.POSIXlt("2022-09-01 13:52:00", tz='UTC') & data$date<as.POSIXlt("2022-09-01 14:21:00", tz='UTC'),
  ]
...
p2 <- ggplot()+
    geom_tile(les, mapping = aes(x = lon, y = lat), height = 30/per.lat, width = 30/per.lon(les$lat), fill = "red")+
      geom_path(n2o, mapping = aes(x = lon, y = lat), group = 1)


tiff("D:/1 PhD Studies/1 Data/LES/0901/plots/les&ac.tiff",  units="mm", width=150, height=75, res=300)
...
tiff("D:/1 PhD Studies/1 Data/LES/0901/plots/les&ac.3d.tiff",  units="mm", width=300, height=200, res=300)
...
les.temp <- read.csv("D:/1 PhD Studies/1 Data/LES/0901/wind/temp.csv")
les.pre <- read.csv("D:/1 PhD Studies/1 Data/LES/0901/wind/pressure.csv") # unit of pressure is Pa and unit of temp is kelvin
...
tiff("D:/1 PhD Studies/1 Data/LES/0901/plots/time.series.les&ac.temp.tiff",  units="mm", width=150, height=150, res=300)
...
tiff("D:/1 PhD Studies/1 Data/LES/0901/plots/time.series.les&ac.pressure.tiff",  units="mm", width=150, height=150, res=300)
grid.arrange(p1,p2, nrow =2, top = "without assimilation")
while (!is.null(dev.list()))  dev.off()
...
# n2o
condition <- transect6$distance<(-7500) | (transect6$distance>0 & transect6$distance<5000)
ac.sel <- subset(transect6, condition)
lm <- lm(N2O.ac.ppb~distance, ac.sel); summary(lm)
intercept <- coefficients(lm)[[1]]; slope <- coefficients(lm)[[2]]
transect6 <- mutate(transect6, bg = intercept+slope*distance)
transect6 <- mutate(transect6, n2o = N2O.ac.ppb - bg)

les$date <- as.POSIXct(les$date, tz = "UTC");
les <- les[les$date>=as.POSIXct("2022-09-06 12:39:", tz = "UTC"),]
transect6$date <- as.POSIXct(transect6$date, tz = "UTC")

transect6.sel <- merge(transect6, data.sel, by = "date")

p1 <- ggplot()+
  geom_line(transect6, mapping = aes(x = date, y = n2o))+
  #geom_point(transect6.sel, mapping = aes(x = date, y = n2o), colour = "gold")+
...
# ch4
condition <- transect6$distance>(-5000) & transect6$distance<5000
ac.sel <- subset(transect6, condition)
bg <- mean(ac.sel$CH4.ac.ppb) # use mean as bg
transect6 <- cbind(transect6, bg=bg, ch4 = transect6$CH4.ac.ppb - bg)
...
ggplot()+
  geom_point(transect6, mapping =aes(x = distance, y = CH4.ac.ppb))+
...
ggplot()+
  geom_point(transect6, mapping = aes(x = date, y = ch4))+
  geom_point(transect6.sel, mapping = aes(x = date, y = ch4), colour = "gold")+
...
############################
sapply(list.files(pattern="[.]R$", path="c:/Users/xin/Dropbox/xin/Rscripts/functions", full.names=TRUE), source)
data <- read.csv("D:/1 PhD Studies/1 Data/Rotterdam_campaign_2022/AirCore measurements datasets/AirCore_N2O_CO_CO2_CH4_CO_20220901.csv")
...
df.0901 <- mutate(data, flag = case_when(
  ...
  transect.1and2 <- df.0901[which(df.0901$flag=='2nd.transect' | df.0901$flag=='1st.transect'),]
  transect6 <- df.0901[which(df.0901$flag=='2nd.transect'),]
  
#################################### pressure & temperature ##################################################
temp <- read.csv("D:/1 PhD Studies/1 Data/LES/0901/extract_les/temp.ac.csv");
pre <- read.csv("D:/1 PhD Studies/1 Data/LES/0901/extract_les/pressure.ac.csv");
...
ggplot()+
  ...
  geom_point(transect6, mapping = aes(x = lat, y = Ps), colour = "gold")+
  #geom_point(les.mean, mapping = aes(x = lat, y = p))+
  theme(legend.title = element_blank(), legend.position = "bottom")

ggplot()+
  geom_point(les, mapping = aes(x = lat, y = e, group = date, colour = date))+
  geom_line(transect6, mapping = aes(x = lat, y = p.H2O), colour = "gold")+
...
ggplot()+
  ...
  geom_line(transect6, mapping = aes(x = lat, y = Tair), colour = "gold")+
...
ggplot()+
  ...
  geom_line(transect6, mapping = aes(x = lat, y = pot), colour = "gold")+
...
###################################### wind ######################################
wind <- read.csv("D:/1 PhD Studies/1 Data/LES/0901/extract_les/wind.ac.csv");
...
ggplot()+
  ...
  geom_line(transect6, mapping = aes(x = lat, y = ws), colour = "gold")+
...
ggplot()+
  ...
  geom_line(transect6, mapping = aes(x = lat, y = wd), colour = "gold")+
...
les <- read.csv("D:/1 PhD Studies/1 Data/LES/0901/extract_les/plume.ac.csv") # UNIT is ppb for N2O and CH4
les.mean <- group_by(les, lat, layer) %>% summarise(
...
# n2o
condition <- transect6$distance<(-10000) | (transect6$distance>(-5000) & transect6$distance<5000)
ac.sel <- subset(transect6, condition)
lm <- lm(N2O.ac.ppb~distance, ac.sel); summary(lm)
intercept <- coefficients(lm)[[1]]; slope <- coefficients(lm)[[2]]
transect6 <- mutate(transect6, bg = intercept+slope*distance)
...
ggplot()+
  geom_line(transect6, mapping = aes(x = lat, y = n2o), colour = "gold")+
...
ggplot()+
  ...
  geom_line(transect6, mapping = aes(x = lat, y = ch4), colour = "gold")+
...
les <- read.csv("D:/1 PhD Studies/1 Data/LES/0901/extract_les/plume.ac.add30.csv") # UNIT is ppb for N2O and CH4
les$date <- as.POSIXct(les$date, tz = "UTC")
...
ggplot()+
  geom_line(transect6, mapping = aes(x = date, y = n2o), colour = "gold")+
...
ggplot()+
  geom_line(transect6, mapping = aes(x = date, y = ch4), colour = "gold")+
...
################################# instantaneous plume ###############################################
n2o <- read.csv("D:/1 PhD Studies/1 Data/LES/0901/results/n2o.layers.csv")
...
############### plume averaged from different time intervals #################################
 n2o <- read.csv("D:/1 PhD Studies/1 Data/LES/0901/results/n2o.layers.diff.interval.csv")
